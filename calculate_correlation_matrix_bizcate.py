...
def calculate_category_correlation(survey_df, identifying_cols, category_col, value_col, min_periods, ensure_positive_definite=True):

    # Calculate the correlation matrix
    survey_correlation = (
        survey_df
        .pivot(index=identifying_cols, columns=category_col, values=value_col)
        .corr(method="pearson", min_periods=min_periods)
        .fillna(0)
    )

    # Fill the main diagonal with 1s (to ensure low sample subcategories are correct)
    survey_correlation.values[np.diag_indices_from(
        survey_correlation.values)] = 1

    # If specified, correct the correlation matrix to ensure that it's positive definite
    if ensure_positive_definite:
        corrected_survey_correlation = correct_correlation_matrix(
            survey_correlation)
...
def correct_correlation_matrix(cor_matrix, min_value=1e-5):
    ...
    correction_factor = np.matrix(
        np.diag(1 / np.sqrt(np.diag(corrected_cor_matrix))))

    # Apply the correction factor
    corrected_cor_matrix = correction_factor * \
        corrected_cor_matrix * correction_factor.T
...
bizcate_correlation = (
    calculate_category_correlation(
        survey_df=bizcate_think, identifying_cols=["CHANNEL", "CUT_ID", "MONTH_YEAR", "RETAILER_CODE"], category_col="SUB_CODE", value_col="TOTALTHINK_RTS", min_periods=3500)
    .rename(columns={"SUB_CODE": "BIZCATE_CODE", "SIMILAR_SUB_CODE": "SIMILAR_BIZCATE_CODE"})
)
...
formatted_bizcate_correlation = (
    bizcate_correlation
    .merge(bizcate_mapping, how="left", on="BIZCATE_CODE")
    .merge(
        bizcate_mapping
        .rename(columns={"BIZCATE": "SIMILAR_BIZCATE", "BIZCATE_CODE": "SIMILAR_BIZCATE_CODE"}),
        how="left", on="SIMILAR_BIZCATE_CODE"
    )
    >> select(_.BIZCATE_CODE, _.BIZCATE, _.SIMILAR_BIZCATE_CODE, _.SIMILAR_BIZCATE, _.CORRELATION)
)
